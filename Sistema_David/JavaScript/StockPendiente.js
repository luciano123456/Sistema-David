    ...
    try {
        ...
        if (result != null) {
            for (let i = 0; i < result.data.length; i++) {
                ...
                var cardId = result.data[i].Id; // Obtener el ID del resultado
                newCard.id = cardId; // Asignar el ID al elemento newCard
                newCard.classList.add('card', 'mb-3', 'position-relative');
                newCard.innerHTML = `
                        <div class="half-blue">
                        <span class="texto-titulo text-white">${userSession.IdRol === 1 ? result.data[i].Usuario : ''}</span>
                        ...
                        ${userSession.IdRol === 1 && result.data[i].Estado === "Pendiente" ?
                    ...
                    <div class="mt-2 text-center">
                        ...
                        ${(userSession.Id === result.data[i].IdUsuario && result.data[i].Estado === 'Pendiente') ?
                        `<div class="botones-row mt-4 row justify-content-center">
                                <button class="btn btn-success col-5 me-2 mb-2" onclick="aceptarStock(${cardId})"><i class="fa fa-check"></i> Aceptar</button>
                                <button class="btn btn-danger col-5 mb-2" onclick="rechazarStock(${cardId})"><i class="fa fa-times"></i> Rechazar</button>
                            </div>` : ''}
                    </div>
                `;
                ...
                if (result.data[i].ImagenProducto == null) {
                    imageUrl = "/Imagenes/productodefault.png";
                } else {
                    imageUrl = 'data:image/png;base64,' + result.data[i].ImagenProducto;
                }
