...
namespace Sistema_David.Models.Modelo
{
    public class ClientesModel
    {
        ...
        public static Cliente BuscarCliente(int id)
        {
            using (Sistema_DavidEntities db = new Sistema_DavidEntities())
            {
                var query = @"SELECT c.Id, c.Nombre, c.Apellido, c.Fecha, c.Dni, c.Direccion, c.Telefono, c.IdEstado, c.IdZona, z.Nombre as Zona, ec.Nombre as Estado, c.IdVendedor, u.Nombre as Vendedor, COALESCE(s.Saldo, 0) AS Saldo 
                      FROM Clientes c 
                      INNER JOIN EstadosClientes ec ON c.IdEstado = ec.Id 
                      INNER JOIN Usuarios u ON c.IdVendedor = u.Id 
					  INNER JOIN Zonas z on c.IdZona = z.Id
        ...
        public static Cliente BuscarCliente(string documento)
        {
            using (Sistema_DavidEntities db = new Sistema_DavidEntities())
            {
                ...
                var user = (from d in db.Clientes
                         .SqlQuery("select c.Id, c.Nombre, c.Apellido, c.Fecha, c.Dni, c.Direccion, c.Telefono, c.IdEstado,  c.IdZona, ec.Nombre as Estado, ec.Nombre, c.IdVendedor, u.Nombre as Vendedor from Clientes c inner join EstadosClientes ec on c.IdEstado = ec.Id inner join Usuarios u on c.IdVendedor  = u.Id")
                            select new Cliente
                            {
                                Id = d.Id,
                                Nombre = d.Nombre,
