...
namespace Sistema_David.Controllers
{
    ...
    [Authorize]
    [CheckBloqueoSistema]
    public class ClientesCeroController : Controller
    {
        ...
        public ActionResult AsignarVendedor(int idVendedor, string clientes)
        {
            try
            {
                var lstClientes = JsonConvert.DeserializeObject<List<int>>(clientes);

                var result = ClientesCeroModel.AsignarVendedor(lstClientes, idVendedor);
        ...
        public ActionResult Listar(int idVendedor, string Nombre, string Apellido, string Dni, int idZona, int idVendedorAsignado)
        {
            ...
            {
                UsuariosModel.setClientesCero(idVendedor, 0);
            }

            if (SessionHelper.GetUsuarioSesion() != null && (SessionHelper.GetUsuarioSesion().IdRol != 1 && SessionHelper.GetUsuarioSesion().IdRol != 4)) //TODOS MENOS ADMIN Y COMPROBANTES
            {
                
                result = ClientesCeroModel.ListaClientes(-1, Nombre, Apellido, Dni, idZona, SessionHelper.GetUsuarioSesion().Id);

            }
            else
            {
                result = ClientesCeroModel.ListaClientes(idVendedor, Nombre, Apellido, Dni, idZona, idVendedorAsignado);
            }
